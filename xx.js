// import { getCourses } from "./courses.js";
export const courses = {
  "104 سلم": [
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "180",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 03:00 م - 04:50 م ",
          timee: {
            startHour: "15",
            startMin: "00",
            endHour: "16",
            endMin: "50",
          },
          class: " 4062 2 3 و 4",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "812",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["4"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " علوم 01 2 ج 5",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1250",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "41",
      periods: [
        {
          day: ["3"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4055 01 1 ج 2 ج 12",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1251",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "41",
      periods: [
        {
          day: ["3"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4055 01 1 و 2 و 7",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1348",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["5"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " علوم 01 3 و 8",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1424",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["5"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " علوم 01 2 د 1",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1526",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4034 03 1 f 043",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "1877",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4034 03 1 f 043",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "2384",
      courseHours: "2",
      open: true,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 54 01 2 C CS26",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "104 سلم",
      courseTitle: "اسس النظام السياسى فى الاسلام",
      classCode: "2779",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " علوم 01 2 و 3",
          classType: "محاضرة",
        },
      ],
    },
  ],
  "2540 ريض": [
    {
      courseCode: "2540 ريض",
      courseTitle: "الطرائق العددية",
      classCode: "2363",
      courseHours: "4",
      open: true,
      instructor: "لم يحدد من الكلية",
      exam: "13",
      periods: [
        {
          day: ["2"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 a as33 ",
          classType: "محاضرة",
        },
        {
          day: ["1"],
          time: " 11:00 ص - 11:50 ص ",
          timee: {
            startHour: "11",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 a as33",
          classType: "محاضرة",
        },
        {
          day: ["3"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 C CS26",
          classType: "عملي",
        },
      ],
    },
  ],
  "3501 عال": [
    {
      courseCode: "3501 عال",
      courseTitle: "مقدمة في الذكاء الاصطناعي",
      classCode: "2370",
      courseHours: "4",
      open: true,
      instructor: "احمد عمر احمد عسيري",
      exam: "8",
      periods: [
        {
          day: ["2"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 54 01 2 B BS1 ",
          classType: "محاضرة",
        },
        {
          day: ["5"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 B BS1",
          classType: "محاضرة",
        },
      ],
    },
  ],
  "2401 عال": [
    {
      courseCode: "2401 عال",
      courseTitle: "النظرية الحسابية",
      classCode: "2368",
      courseHours: "4",
      open: true,
      instructor: "خالد محمد الصادق بسيس",
      exam: "3",
      periods: [
        {
          day: ["1", "3"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 54 01 2 C CS2",
          classType: "محاضرة",
        },
        {
          day: ["4"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 C CS1",
          classType: "تمارين",
        },
      ],
    },
  ],
  "3001 عال": [
    {
      courseCode: "3001 عال",
      courseTitle: "الممارسات الأخلاقية والمهنية",
      classCode: "2385",
      courseHours: "3",
      open: true,
      instructor: "عبدالاله الوابل",
      exam: "13",
      periods: [
        {
          day: ["3"],
          time: " 10:00 ص - 10:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "10",
            endMin: "50",
          },
          class: " 54 01 2 C CS5 ",
          classType: "محاضرة",
        },
        {
          day: ["2"],
          time: " 03:00 م - 04:50 م ",
          timee: {
            startHour: "15",
            startMin: "00",
            endHour: "16",
            endMin: "50",
          },
          class: " 54 01 2 C CS5",
          classType: "محاضرة",
        },
      ],
    },
  ],
  "2111 هاب": [
    {
      courseCode: "2111 هاب",
      courseTitle: "أسس هندسة البرمجيات",
      classCode: "2379",
      courseHours: "4",
      open: false,
      instructor: "محمد شاه محمد كنقو",
      exam: "1",
      periods: [
        {
          day: ["2"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 C CS27 ",
          classType: "محاضرة",
        },
        {
          day: ["1"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 54 01 2 C CS27",
          classType: "محاضرة",
        },
      ],
    },
  ],
  "3821 عال": [
    {
      courseCode: "3821 عال",
      courseTitle: "برمجة تطبيقات الويب",
      classCode: "2382",
      courseHours: "4",
      open: true,
      instructor: "بلحسن عبدالمجيد خالد العكروت",
      exam: "17",
      periods: [
        {
          day: ["3"],
          time: " 08:00 ص - 08:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "08",
            endMin: "50",
          },
          class: " 54 01 1 B BF2 ",
          classType: "محاضرة",
        },
        {
          day: ["1"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 54 01 1 B BF2",
          classType: "محاضرة",
        },
        {
          day: ["5"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 54 01 1 B BF3",
          classType: "عملي",
        },
      ],
    },
  ],
  "3791 هال": [
    {
      courseCode: "3791 هال",
      courseTitle: "معمل نظم شبكات الحاسب",
      classCode: "2371",
      courseHours: "2",
      open: true,
      instructor: "جابر حسن علي العوفي",
      exam: "41",
      periods: [
        {
          day: ["5"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 54 01 1 A AF6 ",
          classType: "عملي",
        },
        {
          day: ["2"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 1 A AF6",
          classType: "عملي",
        },
      ],
    },
  ],
  "3801 عال": [
    {
      courseCode: "3801 عال",
      courseTitle: "اساسيات الامن السيبراني",
      classCode: "2376",
      courseHours: "4",
      open: true,
      instructor: "بندر صالح محمد المصلوخ",
      exam: "22",
      periods: [
        {
          day: ["2", "4"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 54 01 2 a as32",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "3801 عال",
      courseTitle: "اساسيات الامن السيبراني",
      classCode: "2567",
      courseHours: "4",
      open: true,
      instructor: "عدي محمد سليمان الحضيف",
      exam: "22",
      periods: [
        {
          day: ["1"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: "  ",
          classType: "محاضرة",
        },
        {
          day: ["4"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 54 01 1 A AF10",
          classType: "محاضرة",
        },
      ],
    },
  ],
  "106 سلم": [
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2915",
      courseHours: "2",
      open: false,
      instructor: "عبدالله محمد عبدالله العسكر",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 4035 01 g A AA17",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2916",
      courseHours: "2",
      open: false,
      instructor: "عبدالرحمن عبدالعزيز عيسى الجريوى",
      exam: "12",
      periods: [
        {
          day: ["1"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 35 01 G D GO063",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2917",
      courseHours: "2",
      open: false,
      instructor: "صلاح علي عبدالله الزيات",
      exam: "12",
      periods: [
        {
          day: ["5"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " علوم 01 2 و 2",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2918",
      courseHours: "2",
      open: false,
      instructor: "البدري عبدالمجيد احمد سالم",
      exam: "12",
      periods: [
        {
          day: ["1"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 4035 01 g A AA17",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2919",
      courseHours: "2",
      open: false,
      instructor: "بدوي علي السيد زلهف",
      exam: "12",
      periods: [
        {
          day: ["5"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 4035 01 2 B 2B136",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2921",
      courseHours: "2",
      open: false,
      instructor: "سعد عبيد مطلق الرفدي",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " 35 01 F A FO109",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2922",
      courseHours: "2",
      open: false,
      instructor: "صلاح علي عبدالله الزيات",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " علوم 01 2 و 7",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2923",
      courseHours: "2",
      open: false,
      instructor: "وائل يحيى بخيت الجنيدي عوض",
      exam: "12",
      periods: [
        {
          day: ["3"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " علوم 01 2 و 1",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2924",
      courseHours: "2",
      open: false,
      instructor: "وائل يحيى بخيت الجنيدي عوض",
      exam: "12",
      periods: [
        {
          day: ["4"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " علوم 01 2 و 8",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2926",
      courseHours: "2",
      open: false,
      instructor: "سعد عبيد مطلق الرفدي",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 35 111 1 4 F131",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2927",
      courseHours: "2",
      open: true,
      instructor: "سعد عبيد مطلق الرفدي",
      exam: "12",
      periods: [
        {
          day: ["1"],
          time: " 10:00 ص - 11:50 ص ",
          timee: {
            startHour: "10",
            startMin: "00",
            endHour: "11",
            endMin: "50",
          },
          class: " 54 01 2 A AS1",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2935",
      courseHours: "2",
      open: false,
      instructor: "البدري عبدالمجيد احمد سالم",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 03:00 م - 04:50 م ",
          timee: {
            startHour: "15",
            startMin: "00",
            endHour: "16",
            endMin: "50",
          },
          class: " 4035 01 g A AA17",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2936",
      courseHours: "2",
      open: false,
      instructor: "عبدالله محمد عبدالله العسكر",
      exam: "12",
      periods: [
        {
          day: ["4"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4033 02 0 B BG02",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2937",
      courseHours: "2",
      open: true,
      instructor: "عبدالرحمن عبدالعزيز عيسى الجريوى",
      exam: "12",
      periods: [
        {
          day: ["2"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 54 01 1 A AF10",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2943",
      courseHours: "2",
      open: false,
      instructor: "صلاح علي عبدالله الزيات",
      exam: "12",
      periods: [
        {
          day: ["5"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " علوم 01 2 و 2",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2967",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "",
      periods: [
        {
          day: ["4"],
          time: " 08:00 ص - 09:50 ص ",
          timee: {
            startHour: "08",
            startMin: "00",
            endHour: "09",
            endMin: "50",
          },
          class: " علوم 01 2 ج 4",
          classType: "محاضرة",
        },
      ],
    },
    {
      courseCode: "106 سلم",
      courseTitle: "السيرة النبوية",
      classCode: "2968",
      courseHours: "2",
      open: false,
      instructor: "لم يحدد من الكلية",
      exam: "12",
      periods: [
        {
          day: ["4"],
          time: " 01:00 م - 02:50 م ",
          timee: {
            startHour: "13",
            startMin: "00",
            endHour: "14",
            endMin: "50",
          },
          class: " 4033 02 1 D DF08",
          classType: "محاضرة",
        },
      ],
    },
  ],
};

console.log(getCourses()["106 سلم"].find);
